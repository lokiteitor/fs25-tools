...
# 1. North connection (straight vertical from y=512 to y=752)
road_pts = [(3752.0, y) for y in np.linspace(512, 752, 200)]

# 2. Middle winding road (spline from y=752 to y=3452)
Y_mid = np.linspace(752, 3452, 1000, dtype=np.float32)
X_mid = cs(Y_mid).astype(np.float32)
road_pts.extend([(x, y) for x, y in zip(X_mid, Y_mid)])

# 3. South connection (straight vertical from y=3452 to y=3584)
road_pts.extend([(352.0, y) for y in np.linspace(3452, 3584, 200)])
